...
select top 5 p.Name,SUM(sod.LineTotal) as [Total Sales]
from Product as p join SalesOrderDetail as sod
...
select pc.Name,AVG(sod.UnitPrice) as [Average Price]
from SalesOrderDetail sod join Product as p 
on p.ProductID=sod.ProductID join ProductSubcategory as psc on p.ProductSubcategoryID=psc.ProductSubcategoryID
...
select p.Name,SUM(sod.OrderQty) as [Amount Ordered],pc.Name
from SalesOrderDetail sod join Product as p 
on p.ProductID=sod.ProductID join ProductSubcategory as psc on p.ProductSubcategoryID=psc.ProductSubcategoryID
join ProductCategory as pc on psc.ProductCategoryID=pc.ProductCategoryID
...
--Added pc.Name in the Select Line to make sure the Category Name isn't 'Clothing' or 'Components'.

--Ex4:

select top 3 st.Name,SUM(soh.TotalDue) as [Sum Of Sales]
from SalesOrderHeader as soh join SalesTerritory as st
on soh.TerritoryID=st.TerritoryID
group by st.Name
order by [Sum Of Sales] DESC;

--Added SUM(soh.TotalDue) as [Sum Of Sales] to show the numbers, and clarify the answer.

--Ex5:

select  p.BusinessEntityID,p.FirstName+' '+p.LastName as [Full Name],t.[Sum Of Orders]
from Person as p  right join

(select c.CustomerID, COUNT(soh.SalesOrderID) as [Sum Of Orders]
	from Customer as c left join SalesOrderHeader as soh  
...
--Added t.[Sum Of Orders] to make sure the Sum of Orders equal to 0
...
from SalesPerson as sp join SalesTerritory as st
on sp.TerritoryID=st.TerritoryID
group by st.TerritoryID
having COUNT(sp.BusinessEntityID)=0) ;


--Note: No Territory line deleted.
...
insert into SalesTerritory ([TerritoryID],[Name],[CountryRegionCode],[Group],[SalesYTD],[SalesLastYear],[CostYTD],[CostLastYear],[rowguid],[ModifiedDate])
		...
		from [AdventureWorks2022].[Sales].[SalesPerson] as sp join [AdventureWorks2022].[Sales].[SalesTerritory] as st
		on sp.TerritoryID=st.TerritoryID
		group by st.TerritoryID,st.[Name],st.CountryRegionCode,st.[Group],st.SalesYTD,st.SalesLastYear,st.CostYTD,st.CostLastYear,st.rowguid,st.ModifiedDate
		having COUNT(sp.BusinessEntityID)=0);

--After Using use this code and comment it:

--set identity_insert SalesTerritory off;

--(Note that the Terriotory ID is Identity, so we should disbale the insertion and enable it after the insertion.)

------------------------------------

--Ex8

select p.FirstName+' '+p.LastName as [Full Name], t.CustomerID,t.[Num of Orders]
from Person as p join 

		(select c.CustomerID,COUNT(soh.SalesOrderID) as [Num of Orders]
		from SalesOrderHeader as soh join Customer as c
...
-- Added t.CustomerID,t.[Num of Orders] to validate their Customer Id and the calculated Number of orders they made.

---------------------

--Ex9:

select GroupName,COUNT(DepartmentID) as [Count Of Dep]
from Department
group by GroupName
...
select p.FirstName+' '+p.LastName as [Full Name],d.Name,s.Name,YEAR(e.HireDate) as [Hire Year],d.GroupName
from Employee as e join EmployeeDepartmentHistory edh
on e.BusinessEntityID=edh.BusinessEntityID join Department as d
on edh.DepartmentID=d.DepartmentID 
join [Shift] as s on edh.ShiftID=s.ShiftID
join Person as p on e.BusinessEntityID=p.BusinessEntityID
where YEAR(e.HireDate)>2010 and (d.GroupName in ('Quality Assurance','Manufacturing'));


--Added YEAR(e.HireDate) as [Hire Year],d.GroupName to validate that the workers hired after 2010 and their Group Name is
--one of these : 'Quality Assurance','Manufacturing'.